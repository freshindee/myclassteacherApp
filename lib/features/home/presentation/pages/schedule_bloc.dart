...
class ScheduleBloc extends Bloc<ScheduleEvent, ScheduleState> {
  ...
  Future<void> _onLoadGrades(
    ...
    final result = await getAvailableGrades(event.schoolId);
    result.fold(
  ...
  Future<void> _onLoadTimetable(
    ...
    emit(ScheduleLoading());
    final result = await getTimetableByGrade(GetTimetableByGradeParams(schoolId: event.schoolId, grade: event.grade));
    result.fold(
      (failure) => emit(ScheduleError(failure.toString())),
